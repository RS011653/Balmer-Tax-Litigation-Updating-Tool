...
def get_settings() -> Settings:
    ...
    return Settings(
        ...
        ADMIN_EMPLOYEE_ID=str(s.get("ADMIN_EMPLOYEE_ID", "11653")),
        ADMIN_NAME=s.get("ADMIN_NAME", "Raja Saha"),
        SESSION_TIMEOUT_MINUTES=int(s.get("SESSION_TIMEOUT_MINUTES", 30)),
        APP_VERSION=s.get("APP_VERSION", "1.0.0"),
        GOOGLE_SHEET_WEBHOOK_URL=s.get("GOOGLE_SHEET_WEBHOOK_URL", ""),
    )
...
st.markdown(
    ...
    .hero {
        ...
        background: linear-gradient(135deg, #2563eb 0%, #0ea5e9 42%, #8b5cf6 100%);
    ...
    .feature-card {
        ...
        border: 1px solid rgba(148,163,184,0.22); box-shadow: 0 18px 44px rgba(15,23,42,0.08);
        color: #0f172a;
    }
    ...
    .stButton button, .stDownloadButton button {
        ...
        border-radius: 16px !important; padding: 0.78rem 1.15rem !important;
...
def render_footer() -> None:
    st.markdown(
        '<div class="footer-note">This app developed by Raja Saha, Sr Manager Taxation, '
        'Balmer Lawrie &amp; Co Ltd</div>',
        unsafe_allow_html=True,
    )
...
def begin_action_once(action_key: str, cooldown_seconds: float = 3.0) -> bool:
    now = time.time()
...
def notify_admin(
    ...
    if str(actor_id).strip() == settings.ADMIN_EMPLOYEE_ID:
        actor_name = settings.ADMIN_NAME
        actor_email = settings.ADMIN_EMAIL
    ...
    if details:
        detail_rows = "".join(
            f"<tr><td style='padding:10px 14px;border:1px solid #dbe7ff;font-weight:700;color:#1e3a8a;background:#f8fbff'>{k}</td>"
            f"<td style='padding:10px 14px;border:1px solid #dbe7ff;color:#0f172a;background:#ffffff'>{v}</td></tr>"
...
def sync_to_google_sheet(row: dict) -> None:
    ...
    try:
        payload = {
            "case_ref": row.get("case_ref") or row.get("id"),
            ...
            "employee_email": row.get("employee_email", st.session_state.get("email", "")),
        }
        requests.post(settings.GOOGLE_SHEET_WEBHOOK_URL, json=payload, timeout=15)
    except Exception:
        pass
...
def logout() -> None:
    for key in ["authenticated", "employee_id", "employee_name", "role", "division", "email", "last_active_time"]:
...
def is_admin() -> bool:
    return st.session_state.get("role") == "admin" or str(st.session_state.get("employee_id")) == settings.ADMIN_EMPLOYEE_ID
...
def safe_employee_records_from_df(df: pd.DataFrame) -> list[dict]:
    ...
    df.columns = [str(c).strip() for c in df.columns]
    alias_map = {
        "Employee ID": "employee_id", "Employee Name": "employee_name", "Name": "employee_name",
        "Email": "email", "Email ID": "email", "Division": "division", "Division Name": "division",
        "Role": "role", "Is Active": "is_active", "Active": "is_active",
    }
...
def import_employee_file(uploaded_file, default_password: str = "Welcome123") -> tuple[bool, str]:
    try:
        df = pd.read_excel(uploaded_file)
        ...
        for rec in records:
            ...
            if existing.data:
                ...
                inserted += 1
        details = {"rows": len(records), "inserted": inserted, "updated": updated, "skipped": skipped}
        save_audit("import_employees", "employees", f"{len(records)} rows", details)
        notify_admin("import_employees", "employees", f"{len(records)} rows", details)
        return True, f"Employee import complete. Inserted {inserted}, Updated {updated}, Skipped {skipped}."
    except Exception as exc:
        return False, f"Employee import failed: {exc}"


def add_employee_manual(payload: dict, default_password: str = "Welcome123") -> tuple[bool, str]:
    try:
    ...
    with col1:
        st.markdown(
            ...
            <div class="feature-card">
              ...
              <p>This version adds automatic due-date reminders and safer save handling.</p>
              ...
              <div class="feature-item">Hearing reminders are sent automatically 7 and 2 days before due date.</div>
            </div>
            """,
            unsafe_allow_html=True,
        )
    with col2:
        ...
        with tab1:
            with st.form("login_form"):
                ...
                if submitted:
                    if not begin_action_once("login_click", cooldown_seconds=2.5):
...
def render_dashboard() -> None:
    ...
    with left:
        ...
        fig = px.bar(tax, x="tax_type", y="cases", color="tax_type")
    ...
    with right:
        ...
        fig2 = px.pie(stat, names="current_status", values="cases", hole=0.56)
...
def render_litigation() -> None:
        ...
        if df.empty:
            ...
            if editable_options:
                ...
                with st.form("edit_litigation_form"):
                    ...
                    employee_id = c2.text_input("Employee ID", value=str(selected_row.get("employee_id") or ""), disabled=not is_admin())
                    ...
                    last_hearing_date = e2.text_input("Last Hearing Date (YYYY-MM-DD)", value=str(selected_row.get("last_hearing_date") or ""))
                    next_hearing_date = e3.text_input("Next Hearing Date (YYYY-MM-DD)", value=str(selected_row.get("next_hearing_date") or ""))
                    ...
                    if submitted_edit:
                        if not begin_action_once("edit_litigation_click", cooldown_seconds=3.0):
                            st.warning("Please wait, your previous update is still processing.")
                        else:
                            payload = {
    ...
    with tabs[1]:
        with st.form("add_litigation_form"):
            ...
            if submitted:
                if not begin_action_once("add_litigation_click", cooldown_seconds=3.0):
                    st.warning("Please wait, your previous request is still processing.")
                else:
                    payload = {
...
def render_profile() -> None:
    render_hero("Profile", "Your account details as stored in the employee master.")
    res = sb.table("employees").select("*").eq("employee_id", st.session_state.get("employee_id")).execute()
    if res.data:
        user = res.data[0]
        st.write(f"**Employee ID:** {user.get('employee_id')}")
        st.write(f"**Name:** {user.get('employee_name')}")
        st.write(f"**Email:** {user.get('email')}")
        st.write(f"**Division:** {user.get('division')}")
        st.write(f"**Role:** {user.get('role')}")
        st.write(f"**Active:** {user.get('is_active')}")
    render_footer()


def render_admin() -> None:
    render_hero("Admin", "Manage employees: add manually, bulk import, or edit existing records.")
    tab1, tab2, tab3 = st.tabs(["Employee List", "Add Employee", "Import Employees"])

    with tab1:
        emp_df = fetch_table("employees")
        st.dataframe(emp_df, use_container_width=True, hide_index=True)
        if not emp_df.empty:
            edit_options = [f"{r.get('employee_id')} | {r.get('employee_name')}" for _, r in emp_df.iterrows()]
            edit_label = st.selectbox("Select employee to edit", edit_options)
            edit_row = emp_df[emp_df.apply(lambda x: f"{x.get('employee_id')} | {x.get('employee_name')}" == edit_label, axis=1)].iloc[0].to_dict()
            with st.form("edit_employee_form"):
                employee_name = st.text_input("Employee Name", value=edit_row.get("employee_name") or "")
                email = st.text_input("Email", value=edit_row.get("email") or "")
                division = st.text_input("Division", value=edit_row.get("division") or "")
                role = st.selectbox("Role", ["user", "admin"], index=0 if edit_row.get("role") != "admin" else 1)
                is_active = st.checkbox("Is Active", value=bool(edit_row.get("is_active", True)))
                submitted_emp_edit = st.form_submit_button("Update Employee", use_container_width=True)
                if submitted_emp_edit:
                    if not begin_action_once("update_employee_click", cooldown_seconds=3.0):
                        st.warning("Please wait, update already in progress.")
                    else:
                        ok, msg = update_employee_manual(edit_row["employee_id"], {
                            "employee_name": employee_name, "email": email,
                            "division": division, "role": role, "is_active": is_active,
                        })
                        if ok:
                            st.success(msg)
                            st.rerun()
    ...
    with tab2:
        with st.form("add_employee_form"):
            ...
            if submitted_add_emp:
                if not begin_action_once("add_employee_click", cooldown_seconds=3.0):
                    st.warning("Please wait, add already in progress.")
                else:
                    ok, msg = add_employee_manual({
                        "employee_id": employee_id, "employee_name": employee_name,
                        "email": email, "division": division, "role": role, "is_active": is_active,
                    })
                    if ok:
                        st.success(msg)
                        st.rerun()
    ...
    with tab3:
        upl_emp = st.file_uploader("Upload employee Excel", type=["xlsx", "xls"], key="employee_import_upload")
        if upl_emp is not None:
            if st.button("Import Employee File", use_container_width=True):
                if not begin_action_once("import_employee_click", cooldown_seconds=4.0):
                    st.warning("Import already in progress, please wait.")
                else:
                    ok, msg = import_employee_file(upl_emp)
                    if ok:
                        st.success(msg)
                        st.rerun()
                    else:
...
def main() -> None:
    ...
    if choice == "Dashboard":
        render_dashboard()
    elif choice == "Litigation":
        render_litigation()
    elif choice == "Documents":
        render_documents()
    elif choice == "Admin" and is_admin():
        render_admin()
    elif choice == "Profile":
        render_profile()
