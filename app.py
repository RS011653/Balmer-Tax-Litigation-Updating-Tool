...
def upload_supporting_document(case_id: Any, file_obj) -> tuple[bool, str]:
    ...
    try:
        ...
        save_audit("upload", "documents", path, {"case_id": case_id, "filename": file_obj.name})
        notify_admin("upload", "documents", path, {"case_id": case_id, "filename": file_obj.name})
...
def delete_document(doc_row: dict) -> tuple[bool, str]:
    try:
        ...
        save_audit("delete_document", "documents", doc_id, {"filename": doc_row.get("filename")})
        notify_admin("delete_document", "documents", str(doc_id), {"filename": doc_row.get("filename")})
